...
import { X, ExternalLink, Play, Image } from 'lucide-react'
...
export default function AdBanner({
  ...
  if (variant === 'video') {
    return (
      <a href="#" onClick={e => e.preventDefault()} data-banner-id={id} className={cn('block cursor-pointer active:scale-[0.98] transition-transform group', className)}>
        <div className={cn('relative rounded-2xl overflow-hidden bg-gradient-to-br from-brand-primary-dark to-brand-blue', ratio || 'aspect-video')}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
            ...
            <span className="text-xs text-white/60 font-medium">{label || 'Video placeholder'}</span>
          </div>
  ...
  if (variant === 'hero') {
    return (
      <a href="#" onClick={e => e.preventDefault()} data-banner-id={id} className={cn('block cursor-pointer active:scale-[0.98] transition-transform group', className)}>
        <div className={cn('relative rounded-2xl border-2 border-dashed border-brand-primary/20 bg-gradient-to-br from-brand-primary/5 to-brand-blue/5 overflow-hidden', ratio || 'aspect-[3/1] sm:aspect-[4/1]')}>
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4">
            <Image size={32} className="text-brand-gray-400/50" />
            <span className="text-[11px] text-brand-gray-400 font-medium text-center">{label || 'BANNER — 1200x400'}</span>
            <div className="flex items-center gap-1 text-[9px] text-brand-gray-400/80"><ExternalLink size={10} /> URL externa</div>
          </div>
  ...
  if (variant === 'twin') {
    return (
      <div className={cn('grid grid-cols-1 sm:grid-cols-2 gap-3', className)}>
        <a href="#" onClick={e => e.preventDefault()} data-banner-id={`${id}-left`} className="block cursor-pointer active:scale-[0.98] transition-transform group">
          <div className="relative rounded-2xl border-2 border-dashed border-brand-primary/20 bg-gradient-to-br from-brand-primary/5 to-brand-blue/5 overflow-hidden aspect-[1.8/1]">
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
              ...
              <span className="text-[10px] text-brand-gray-400 font-medium">BANNER A — 600x340</span>
            </div>
            <div className="absolute inset-0 bg-brand-primary/0 group-hover:bg-brand-primary/5 transition-colors" />
          </div>
        </a>
        <a href="#" onClick={e => e.preventDefault()} data-banner-id={`${id}-right`} className="block cursor-pointer active:scale-[0.98] transition-transform group">
          <div className="relative rounded-2xl border-2 border-dashed border-brand-primary/20 bg-gradient-to-br from-brand-blue/5 to-brand-primary/5 overflow-hidden aspect-[1.8/1]">
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
              <Image size={24} className="text-brand-gray-400/50" />
              <span className="text-[10px] text-brand-gray-400 font-medium">BANNER B — 600x340</span>
            </div>
            <div className="absolute inset-0 bg-brand-primary/0 group-hover:bg-brand-primary/5 transition-colors" />
          </div>
        </a>
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-4">
        <div className="hidden sm:flex w-16 h-16 rounded-xl bg-brand-primary/20 border-2 border-dashed border-brand-primary-dark/20 items-center justify-center shrink-0">
          <span className="text-xs text-brand-primary-dark font-medium text-center leading-tight">AD<br />IMG</span>
        </div>
